...
if __name__ == "__main__":
    ...
    for model_details in models:
        ...
        test_data = build_dataset(
            df_test, eval_dataset_params, model_params, support_bag["tokenizer"])
        test_dl, = build_dataloaders([test_data], [batch_size])

        model = create_model(run_mode, training_params,
                             model_params, support_bag)
        model.load_state_dict(torch.load(model_details["path"]))
        model.to(device)

        metrics = test_loop(test_dl, model, loss_fn, device, log_interval=1000,
                            dataset_type=eval_dataset_params["type"], use_wandb=False)
